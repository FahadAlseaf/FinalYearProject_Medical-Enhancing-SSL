...
PRETRAINED_ENCODER_PATH = "pretrained_encoder.pth" # Loads from Output directory
EPOCHS = 50  # 50 is plenty for results
BATCH_SIZE = 16 # Adjusted for GPU memory
LEARNING_RATE = 1e-4

# --- 1. Dataset (On-the-Fly Generation) ---
class OnFlySRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # Ensure image is at least 224x224
            w, h = hr_img.size
            ...
            # Create LR on the fly (Downsample)
...
class ChannelAttention(nn.Module):
    def __init__(self, c, r=16):
        super().__init__()
        self.pool=nn.AdaptiveAvgPool2d(1); self.fc1=nn.Conv2d(c,c//r,1); self.fc2=nn.Conv2d(c//r,c,1)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self,x): 
        y=torch.cat([torch.max(x,1,keepdim=True)[0],torch.mean(x,1,keepdim=True)],1)
        return x*torch.sigmoid(self.conv(y))

class MIRAMBlock(nn.Module):
    def __init__(self,c):
        super().__init__()
        self.c1=nn.Conv2d(c,c,3,padding=1); self.c2=nn.Conv2d(c,c,3,padding=1)
        self.ca, self.sa = ChannelAttention(c), SpatialAttention()
    def forward(self,x): 
        out=F.relu(self.c1(x)); out=self.c2(out); out=self.ca(out); out=self.sa(out)
        return out+x
...
class GeneratorViT_SR(nn.Module):
    def __init__(self,in_c=3, embed_dim=768, decoder_dim=256, scale=4):
        ...
        self.tail = nn.Sequential(
            UpsampleBlock(decoder_dim, 2), UpsampleBlock(decoder_dim, 2), 
            UpsampleBlock(decoder_dim, 2), UpsampleBlock(decoder_dim, 2),
...
def main():
    ...
    # LOAD SSL WEIGHTS
    try:
        gen.encoder.load_state_dict(torch.load(PRETRAINED_ENCODER_PATH, map_location=device))
        print("✅ SUCCESS: Loaded SSL Pre-trained Weights!")
    except:
        print("⚠️ WARNING: Could not load weights. Training from scratch.")
    ...
    for epoch in range(EPOCHS):
        ...
        for lr, hr in pbar:
            ...
            with torch.cuda.amp.autocast():
                sr = gen(lr).detach()
                loss_d = (F.binary_cross_entropy_with_logits(disc(hr), torch.ones_like(disc(hr))) + 
                          F.binary_cross_entropy_with_logits(disc(sr), torch.zeros_like(disc(sr)))) * 0.5
